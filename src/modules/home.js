...
class Home {
    constructor(Database) {
        ...
        const searchCandidates = async () => {
            ...
            let suitableCandidates = (await this.database.select('candidate'))
                ...
                .filter(({name, surname, middlename, position}) => {
                    ...
                    return nameFieldValue.split(' ').every((value) => fullName.includes(value)) && position.toLowerCase().includes(positionFiledValue);
                });
        ...
        (async () => {
            ...
            let candidatesPositions = [...new Set((await this.getCandidatesData()).map((candidate) => candidate.position))].sort((a, b) => a - b);
    ...
    async createPages(candidatesData) {
        ...
        const createCorrectTitle = (num) => {
            ...
            else if (/^[5,6,7,8,9]$|1[1,2,3,4]$|[0,5,6,7,8,9]$/.test(num)) return `${num} Кандидатов`;
        };
        ...
        [...pageBtnsContainer.children].forEach((pageBtn) => pageBtnsContainer.removeChild(pageBtn));
